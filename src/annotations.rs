    ...
    if !origin.is_none() {
        ...
        if origin.is(&tools.types.literal) {
            ...
            if let Ok(origin_type) = &origin_type_result {
                ...
                let (protocol_base_opt, _bindings_opt) =
                    extract_protocol_from_orig_bases(&origin_type, tools)
                        .ok()
                        .flatten()
                        .map(|(p, b)| (Some(p), b))
                        .unwrap_or((None, None));
                ...
                if origin_type.is_subclass(&tools.types.abc_mapping)? {
                    let (items_validator, requires_owner) = if should_use_type_args
                        && let Ok((key_arg, val_arg)) = args.extract()
                    {
                        let (key_validator, key_info) = build_validator_from_annotation(
                            PyString::new(py, &format!("{name}-key")).cast()?,
                            &key_arg,
        ...
        // Bare type annotation (not type[X]) - accept any type object
        // (subclass of object).
