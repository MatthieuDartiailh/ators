...
pub fn create_ators_specialized_alias<'py>(
    ...
    let specialized = crate::class::generic::create_ators_specialized_subclass(cls, params)?;
    let specialized_type = specialized.cast::<PyType>()?;
    wrap_ators_specialized_class(specialized_type)
}

pub(crate) struct ClassInfoStore {
    pub(crate) definitive: HashMap<usize, Arc<AtorsClassInfo>>,
    pub(crate) temporary: HashMap<String, Arc<AtorsClassInfo>>,
    pub(crate) pending_specialization_bindings: HashMap<String, Py<PyDict>>,
    pub(crate) alias_by_specialized: HashMap<usize, Py<PyAny>>,
    pub(crate) specialized_by_alias: HashMap<usize, Py<PyType>>,
}
...
#[inline]
pub(crate) fn get_class_info_store<'py>(py: pyo3::Python<'py>) -> &'py RwLock<ClassInfoStore> {
    CLASS_INFO_STORE.get_or_init(py, || RwLock::new(ClassInfoStore::new()))
}

#[inline]
pub(crate) fn class_key(cls: &Bound<'_, PyType>) -> usize {
    cls.as_ptr().addr()
}
